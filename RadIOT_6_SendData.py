#!/usr/bin/python
# Author: Nathan Cross MD MS, Po-Hao Chen MD MBA
# Copyright (c) 2017 Nathan Cross, Po-Hao Chen MD MBA
...
led2 = LED(led2_pin)
reed_pin = 5
reed = Button(reed_pin, pull_up=True, bounce_time=1)
...
client_id = "RadIOT_phone1" # Customized unique name for your device
server = "iot.eclipse.org"  # Eclipse's free test server
location = "testLocation"   # Customized unqiue name for your location
...
# wipes display and turns off leds
def clearOutput():
    led1.off()
    led2.off()
    draw.rectangle((0,0,width,height), outline=0, fill=0)
...
def ask_question():
    ...
    while True:
        if button1.is_pressed:
            message = "Button 1 Pressed - " + strftime("  %a, %d %b %Y, %H:%M:%S", localtime())
            print(message)
            mqtt_publish(mqttc, message)
        ...
        if button2.is_pressed:
            message = "Button 2 Pressed - " + strftime("  %a, %d %b %Y, %H:%M:%S", localtime())
            print(message)
            mqtt_publish(mqttc, message)
